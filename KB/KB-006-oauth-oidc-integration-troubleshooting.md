...
ZavaCloud supports OAuth 2.0 and OpenID Connect (OIDC) for third-party application integrations. This article covers common integration issues including token refresh failures, JWKS key rotation problems, and third-party app SSO breakages after certificate renewals.
...
## OAuth Endpoints
```
Authorization: https://auth.ZavaCloud.io/oauth2/authorize
Token:         https://auth.ZavaCloud.io/oauth2/token
UserInfo:      https://auth.ZavaCloud.io/oauth2/userinfo
JWKS:          https://auth.ZavaCloud.io/.well-known/jwks.json
Discovery:     https://auth.ZavaCloud.io/.well-known/openid-configuration
```
...
- Refresh token was already used (ZavaCloud uses single-use refresh tokens with rotation)
...
3. Check if the user's account is still active: ZavaAdmin > Users > search by email
4. Re-authorize the integration: have the user complete the OAuth flow again
5. For client credentials flow: verify the client secret hasn't expired (check ZavaAdmin > Apps > [app] > Credentials)

### JWKS Key Rotation Breaking Integrations
**Symptoms:** Third-party apps that validate ZavaCloud JWT tokens start rejecting all tokens after a key rotation.
...
**Resolution Steps:**
1. Ensure the third-party app fetches JWKS dynamically from `https://auth.ZavaCloud.io/.well-known/jwks.json`
2. The app should cache JWKS keys but refresh when it encounters an unrecognized `kid` in a JWT header
3. ZavaCloud publishes the new key 24 hours before rotating. Set up monitoring on the JWKS endpoint to detect changes
...
**Symptoms:** After ZavaCloud's TLS or SAML signing certificate is renewed, third-party integrations that rely on certificate pinning break.
...
**Resolution Steps:**
1. Download the new certificate from ZavaAdmin > Settings > Identity > Certificates
...
4. Subscribe to ZavaCloud's certificate rotation calendar: ZavaAdmin > Notifications > Certificate Events
...
1. Navigate to ZavaAdmin > Settings > Apps > Register App
...
4. **Set up secret expiration alerts** — ZavaAdmin > Notifications > App Credential Events
