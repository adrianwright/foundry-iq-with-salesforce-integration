...
By default, NimbusCloud sends all notification emails from `notifications@nimbuscloud.io`. This article covers how to configure custom sender addresses per product, set up reply-to routing, and manage bounce handling for custom domains.

## Default Sender Addresses

| Product | Default From Address | Purpose |
|---------|---------------------|---------|
| NimbusHub | `hub-notifications@nimbuscloud.io` | Chat mentions, DM notifications, channel alerts |
| NimbusDocs | `docs-notifications@nimbuscloud.io` | Document shares, comments, version updates |
| NimbusBoard | `board-notifications@nimbuscloud.io` | Task assignments, due dates, status changes |
| NimbusConnect | `connect-notifications@nimbuscloud.io` | Meeting invitations, recordings, schedule changes |
| NimbusVault | `vault-notifications@nimbuscloud.io` | File shares, permission changes, storage alerts |
| NimbusID | `security@nimbuscloud.io` | Login alerts, MFA events, password resets |

## Custom Sender Address Configuration

### Per-Product Custom Addresses
Organizations can replace the default NimbusCloud sender addresses with their own domain:

1. **Prerequisites**: Complete the custom domain setup in KB-010 (SPF, DKIM, DMARC)
2. Navigate to NimbusAdmin > Settings > Email > Sender Addresses
3. For each product, enter the custom sender address:
   - Example: `hub-notifications@nimbuscloud.io` → `collaboration@yourcompany.com`
   - Example: `security@nimbuscloud.io` → `it-security@yourcompany.com`
...
- Default: `NimbusCloud Notifications <notifications@nimbuscloud.io>`
- Custom: `Contoso IT Team <collaboration@contoso.com>`

Configure at: NimbusAdmin > Settings > Email > Sender Addresses > Display Name
...
| Route meeting replies to organizer | Enable "Dynamic Reply-To" for NimbusConnect (uses meeting organizer's email) |

### Dynamic Reply-To
For NimbusConnect meeting invitations, enable dynamic reply-to so that replies go to the meeting organizer instead of a generic mailbox:

1. NimbusAdmin > Settings > Email > NimbusConnect > Reply-To Settings
...
1. Email sent from NimbusCloud (or custom domain)
2. If the recipient's server rejects the email, a **bounce notification** is returned
3. NimbusCloud categorizes the bounce:
...
- NimbusAdmin > Settings > Email > Delivery Dashboard
...
**Resolution:**
1. Verify SPF record includes `include:spf.nimbuscloud.io` (see KB-010)
...
### "Via nimbuscloud.io" Showing in Gmail
**Symptoms:** Gmail recipients see "via nimbuscloud.io" next to the sender address.

**Cause:** DKIM signature `d=` domain doesn't match the From address domain.

**Resolution:**
1. Ensure DKIM is configured for your custom domain (not using NimbusCloud's default DKIM)
2. The DKIM `d=yourcompany.com` must match the From address `sender@yourcompany.com`
3. Re-verify the DKIM record in NimbusAdmin > Settings > Email > Custom Sender Domain > Verify DKIM
...
3. **Enable dynamic reply-to for NimbusConnect** so meeting replies reach the organizer
