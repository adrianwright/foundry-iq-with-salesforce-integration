...
ZavaID is the identity and access management service for the ZavaCloud platform. This article covers single sign-on (SSO) configuration using SAML 2.0 and OIDC, including federation with common identity providers (IdPs) like Azure AD (Entra ID), Okta, and Google Workspace.
...
| LDAP | Legacy directory bind (read-only) | Supported via ZavaID Bridge |

## SAML 2.0 Configuration

### Prerequisites
- ZavaCloud Enterprise or Business plan
- Admin access to both ZavaAdmin and your IdP
- IdP metadata XML or manual configuration values (Entity ID, SSO URL, Certificate)

### Step-by-Step Setup

1. **In ZavaAdmin**, navigate to: Settings > Identity > SSO Configuration
...
6. **Download ZavaCloud SP Metadata** from the configuration page
...
### ZavaCloud SP (Service Provider) Details
```
Entity ID:       https://auth.ZavaCloud.io/saml/metadata/{tenant-id}
ACS URL:         https://auth.ZavaCloud.io/saml/acs/{tenant-id}
SLO URL:         https://auth.ZavaCloud.io/saml/slo/{tenant-id}
...
**Causes:**
- IdP certificate has been rotated but not updated in ZavaAdmin
- Clock skew between IdP and ZavaCloud exceeds 5 minutes
- Response is signed with SHA-1 but ZavaCloud requires SHA-256

**Resolution:**
1. Re-download the IdP signing certificate and upload to ZavaAdmin > SSO > Certificates
2. Verify NTP sync on the IdP server (clock skew must be < 5 minutes)
3. Configure IdP to sign with SHA-256 (ZavaCloud does not accept SHA-1 as of January 2026)

### Error: "User Not Found — No Matching Account"
**Causes:**
- NameID in SAML assertion doesn't match any ZavaCloud user email
- User hasn't been provisioned in ZavaCloud (SCIM not configured or manual provisioning required)
...
2. Verify the user exists in ZavaAdmin > Users with the matching email
...
- Multiple IdPs configured and ZavaCloud can't determine which one to use
- Browser cookies blocked or third-party cookie restrictions

**Resolution:**
1. Verify ACS URL exactly matches: `https://auth.ZavaCloud.io/saml/acs/{tenant-id}` (no trailing slash)
2. If multiple IdPs are configured, set a default IdP or use email-domain-based routing
3. Ensure cookies for `*.ZavaCloud.io` are not blocked by browser privacy settings

## OIDC Configuration

For OIDC-based SSO (preferred for Azure AD / Entra ID and Google Workspace):

1. Navigate to ZavaAdmin > Settings > Identity > SSO Configuration
...
6. Map claims to ZavaCloud attributes (email, name, groups)
7. Test and enable
