...
## Description
David Chen reported discovering 47 user accounts in ZavaAdmin that do not correspond to any active users in their Azure AD (Entra ID) directory. These "ghost" accounts were created by SCIM provisioning and appear to be duplicates of existing users with slightly different email formats. Some ghost accounts have accessed ZavaHub and ZavaDocs, consuming licenses.

## Investigation Notes

**2026-01-28 — Initial Triage (Support Engineer: Rachel Torres)**
- Audited ZavaAdmin user list: 47 accounts sourced from SCIM with no matching Active Directory record
...
- 9 ghost accounts had ZavaDocs files and ZavaBoard task assignments — data merged to real accounts before deletion
- Full SCIM sync ran successfully — all users matched correctly, no new duplicates

**2026-02-05 — Escalation to Engineering**
- Escalated as feature request: ZavaCloud should warn admins when SCIM creates users that closely match existing accounts (fuzzy duplicate detection)
