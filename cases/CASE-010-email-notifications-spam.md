...
## Subject
ZavaCloud notification emails consistently going to spam for all users — custom sender domain configured

## Description
Diane Foster reported that ZavaCloud notification emails (task assignments from ZavaBoard, document shares from ZavaDocs, meeting invites from ZavaConnect) are all being delivered to users' spam/junk folders. Lakeshore recently configured a custom sender domain (`notifications@lakeshoreccu.org`) but notifications started going to spam shortly after activation.
...
- Ran email header analysis on a spam-filtered notification:
  - SPF: Pass ✅ (`include:spf.ZavaCloud.io` present)
  - DKIM: **Fail** ❌ — DKIM signature `d=ZavaCloud.io` doesn't align with From address `notifications@lakeshoreccu.org`
  - DMARC: **Fail** ❌ — DMARC policy `p=quarantine` and neither SPF nor DKIM aligned
- Root cause: DKIM record for `lakeshoreccu.org` was not published. The ZavaCloud-generated DKIM key was provided during setup but Diane hadn't added it to their DNS yet

**2026-02-09 — DNS Records Updated**
- Diane added the DKIM record: `ZavaCloud._domainkey.lakeshoreccu.org TXT "v=DKIM1; k=rsa; p=<key>"`
- DNS propagation confirmed after 2 hours (hosted on Cloudflare)
- Re-verified in ZavaAdmin: SPF ✅, DKIM ✅, DMARC ✅ (all passing)
...
- Gmail no longer showing "via ZavaCloud.io" badge

**2026-02-13 — Resolution Confirmed**
- Diane confirmed all ZavaCloud notifications are landing in inboxes for the past 3 days
- Bounce rate at 0.3% (healthy)
- Advised Diane to monitor the delivery dashboard monthly (ZavaAdmin > Settings > Email > Delivery Dashboard)
